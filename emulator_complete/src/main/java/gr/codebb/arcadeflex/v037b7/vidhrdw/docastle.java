...
public class docastle {
  ...
  static void convert_color_prom(
      char[] palette, char[] colortable, UBytePtr color_prom, int priority) {
    int i, j;
    int p_inc = 0;
    for (i = 0; i < 256; i++) {
      int bit0, bit1, bit2;
  ...
  public static VhConvertColorPromPtr docastle_vh_convert_color_prom =
      new VhConvertColorPromPtr() {
        public void handler(char[] palette, char[] colortable, UBytePtr color_prom) {
          convert_color_prom(palette, colortable, color_prom, 0);
        }
      };
  public static VhConvertColorPromPtr dorunrun_vh_convert_color_prom =
      new VhConvertColorPromPtr() {
        public void handler(char[] palette, char[] colortable, UBytePtr color_prom) {
   ...
   * hardware emulation. *************************************************************************
   */
  public static VhStartPtr docastle_vh_start =
      new VhStartPtr() {
        public int handler() {
          ...
          if ((tmpbitmap1 = bitmap_alloc(Machine.drv.screen_width, Machine.drv.screen_height))
              == null) {
   ...
   * hardware emulation. *************************************************************************
   */
  public static VhStopPtr docastle_vh_stop =
      new VhStopPtr() {
        public void handler() {
  ...
  public static ReadHandlerPtr docastle_flipscreen_off_r =
      new ReadHandlerPtr() {
  ...
  public static ReadHandlerPtr docastle_flipscreen_on_r =
      new ReadHandlerPtr() {
        public int handler(int offset) {
  ...
  public static WriteHandlerPtr docastle_flipscreen_off_w =
      new WriteHandlerPtr() {
  ...
  public static WriteHandlerPtr docastle_flipscreen_on_w =
      new WriteHandlerPtr() {
        public void handler(int offset, int data) {
          flip_screen_w.handler(offset, 1);
        }
   ...
   * *************************************************************************
   */
  public static VhUpdatePtr docastle_vh_screenrefresh =
      new VhUpdatePtr() {
        public void handler(osd_bitmap bitmap, int full_refresh) {
          ...
          for (offs = 0; offs < spriteram_size[0]; offs += 4) {
            ...
            if (sprite_transparency[code] != 0) {
              ...
              copybitmap(
                  bitmap, tmpbitmap, 0, 0, 0, 0, clip, TRANSPARENCY_THROUGH, Machine.pens[257]);
            }
          }
