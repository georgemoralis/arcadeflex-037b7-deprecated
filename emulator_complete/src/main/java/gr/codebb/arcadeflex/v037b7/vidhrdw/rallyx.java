...
public class rallyx {
  ...
  static rectangle radarvisiblearea =
      new rectangle(
          28 * 8, 36 * 8 - 1,
          0 * 8, 28 * 8 - 1);

  static rectangle radarvisibleareaflip =
      new rectangle(
          0 * 8, 8 * 8 - 1,
          0 * 8, 28 * 8 - 1);
   ...
   *
   * <p>Rally X has one 32x8 palette PROM and one 256x4 color lookup table PROM. The palette PROM is
   ...
   *
   * <p>In Rally-X there is a 1 kohm pull-down on B only, in Locomotion the 1 kohm pull-down is an
   * all three RGB outputs.
   *
   * <p>*************************************************************************
   */
  ...
  public static VhConvertColorPromPtr rallyx_vh_convert_color_prom =
  ...
  public static VhConvertColorPromPtr locomotn_vh_convert_color_prom =
      new VhConvertColorPromPtr() {
        public void handler(char[] palette, char[] colortable, UBytePtr color_prom) {
          int i;
          // #define TOTAL_COLORS(gfxn) (Machine.gfx[gfxn].total_colors *
   ...
   * <p>*************************************************************************
   */
  public static VhStartPtr rallyx_vh_start =
      new VhStartPtr() {
        public int handler() {
   ...
   * <p>*************************************************************************
   */
  public static VhStopPtr rallyx_vh_stop =
      new VhStopPtr() {
        public void handler() {
  ...
  public static WriteHandlerPtr rallyx_videoram2_w =
      new WriteHandlerPtr() {
  ...
  public static WriteHandlerPtr rallyx_colorram2_w =
      new WriteHandlerPtr() {
        public void handler(int offset, int data) {
  ...
  public static WriteHandlerPtr rallyx_flipscreen_w =
      new WriteHandlerPtr() {
        public void handler(int offset, int data) {
   ...
   * <p>*************************************************************************
   */
  public static VhUpdatePtr rallyx_vh_screenrefresh =
      new VhUpdatePtr() {
          ...
          for (offs = 0; offs < spriteram_size[0]; offs += 2) {
            sx =
                spriteram.read(offs + 1)
                    + ((spriteram_2.read(offs + 1) & 0x80) << 1)
                    - displacement;
            ...
            drawgfx(
                ...
                sx,
                sy,
                flipscreen != 0 ? spritevisibleareaflip : spritevisiblearea,
                TRANSPARENCY_COLOR,
                0);
          }
  ...
  public static VhUpdatePtr jungler_vh_screenrefresh =
      new VhUpdatePtr() {
        public void handler(osd_bitmap bitmap, int full_refresh) {
          int offs, sx, sy;
          int scrollx, scrolly;
  ...
  public static VhUpdatePtr locomotn_vh_screenrefresh =
      new VhUpdatePtr() {
        public void handler(osd_bitmap bitmap, int full_refresh) {
          int offs, sx, sy;
          ...
          for (offs = videoram_size[0] - 1; offs >= 0; offs--) {
            if (dirtybuffer2[offs] != 0) {
              ...
              drawgfx(
                  tmpbitmap1,
                  Machine.gfx[0],
                  (rallyx_videoram2.read(offs) & 0x7f)
                      + 2 * (rallyx_colorram2.read(offs) & 0x40)
                      + 2 * (rallyx_videoram2.read(offs) & 0x80),
                  rallyx_colorram2.read(offs) & 0x3f,
                  flipx,
                  flipy,
                  8 * sx,
            ...
            if (dirtybuffer[offs] != 0) {
              ...
              drawgfx(
                  ...
                  (videoram.read(offs) & 0x7f)
                      + 2 * (colorram.read(offs) & 0x40)
                      + 2 * (videoram.read(offs) & 0x80),
                  colorram.read(offs) & 0x3f,
                  flipx,
                  flipy,
                  8 * sx,
          ...
          for (offs = 0; offs < spriteram_size[0]; offs += 2) {
            ...
            drawgfx(
                bitmap,
                Machine.gfx[1],
                ((spriteram.read(offs) & 0x7c) >> 2)
                    + 0x20 * (spriteram.read(offs) & 0x01)
                    + ((spriteram.read(offs) & 0x80) >> 1),
                spriteram_2.read(offs + 1) & 0x3f,
                NOT(flipscreen),
                NOT(flipscreen),
                sx,
  ...
  public static VhUpdatePtr commsega_vh_screenrefresh =
      new VhUpdatePtr() {
        public void handler(osd_bitmap bitmap, int full_refresh) {
          int offs, sx, sy;
