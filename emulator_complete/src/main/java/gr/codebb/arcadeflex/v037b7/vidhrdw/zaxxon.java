...
public class zaxxon {
   ...
   *
   * <p>Zaxxon has one 256x8 palette PROM and one 256x4 PROM which contains the color codes to use
   * for characters on a per row/column basis (groups of of 4 characters in the same row). Congo
   * Bongo has similar hardware, but it has color RAM instead of the lookup PROM.
   *
   * <p>The palette PROM is connected to the RGB output this way:
   *
  ...
  public static VhConvertColorPromPtr zaxxon_vh_convert_color_prom =
  ...
  static void create_background(osd_bitmap dst_bm, osd_bitmap src_bm, int col) {
    ...
    for (offs = 0; offs < 0x4000; offs++) {
      ...
      if ((Machine.orientation & ORIENTATION_SWAP_XY)
          == 0) /* leave screenful of black pixels at end */ {
        sy += 256;
      }
      ...
      drawgfx(
          src_bm,
          Machine.gfx[1],
          memory_region(REGION_GFX4).read(offs)
              + 256 * (memory_region(REGION_GFX4).read(0x4000 + offs) & 3),
  ...
  public static VhStartPtr zaxxon_vh_start =
      new VhStartPtr() {
        public int handler() {
  ...
  public static VhStartPtr razmataz_vh_start =
      new VhStartPtr() {
        public int handler() {
          int offs;
   ...
   * <p>*************************************************************************
   */
  public static VhStopPtr zaxxon_vh_stop =
      new VhStopPtr() {
        public void handler() {
      ...
      for (i = 0x1e; i >= 0; i--) {
        ...
        if (spriteram.read(offs + 2) != 0xff) {
          drawgfx(
              ...
              spriteram.read(offs + 2 + 2) & 0x80,
              spriteram.read(offs + 2 + 1) & 0x80,
              ((spriteram.read(offs + 2 + 3) + 16) & 0xff) - 31,
              255 - spriteram.read(offs + 2) - 15,
      ...
      for (offs = spriteram_size[0] - 4; offs >= 0; offs -= 4) {
        if (spriteram.read(offs) != 0xff) {
          drawgfx(
              ...
              spriteram.read(offs + 1) & 0x80,
              spriteram.read(offs + 1) & 0x80, /* ?? */
              ((spriteram.read(offs + 3) + 16) & 0xff) - 32,
              255 - spriteram.read(offs) - 16,
              Machine.visible_area,
              TRANSPARENCY_PEN,
              0);
        }
  ...
  public static VhUpdatePtr zaxxon_vh_screenrefresh =
      new VhUpdatePtr() {
        public void handler(osd_bitmap bitmap, int full_refresh) {
            ...
            if ((Machine.orientation & ORIENTATION_SWAP_XY) != 0) {
              ...
              if (zaxxon_vid_type == CONGO_VID) {
                scroll =
                    1023
                        + 63
                        - (zaxxon_background_position.read(0)
                            + 256 * zaxxon_background_position.read(1));
              } else {
                scroll =
                    2048
                        + 63
                        - (zaxxon_background_position.read(0)
                            + 256 * (zaxxon_background_position.read(1) & 7));
              }
              ...
              for (i = Machine.visible_area.min_x; i <= Machine.visible_area.max_x; i++) {
                ...
                if ((zaxxon_vid_type == ZAXXON_VID || zaxxon_vid_type == FUTSPY_VID)
                  ...
                  copybitmap(
                      bitmap, backgroundbitmap2, 0, 0, -scroll, skew, clip, TRANSPARENCY_NONE, 0);
                } else {
                  copybitmap(
                      bitmap, backgroundbitmap1, 0, 0, -scroll, skew, clip, TRANSPARENCY_NONE, 0);
                }
              ...
              for (i = Machine.visible_area.max_y; i >= Machine.visible_area.min_y; i -= 2) {
                ...
                if ((zaxxon_vid_type == ZAXXON_VID || zaxxon_vid_type == FUTSPY_VID)
                    && ((zaxxon_background_color_bank.read() & 1) != 0)) {
                  copybitmap(
                      bitmap, backgroundbitmap2, 0, 0, skew, scroll, clip, TRANSPARENCY_NONE, 0);
                } else {
                  copybitmap(
                      bitmap, backgroundbitmap1, 0, 0, skew, scroll, clip, TRANSPARENCY_NONE, 0);
                }
          ...
          for (offs = videoram_size[0] - 1; offs >= 0; offs--) {
            ...
            if (zaxxon_vid_type == CONGO_VID) {
              ...
              color =
                  (color_codes.read(sx + 32 * (sy / 4)) & 0x0f)
                      + 16 * (zaxxon_char_color_bank.read(0) & 1);
            }
  ...
  public static VhUpdatePtr razmataz_vh_screenrefresh =
      new VhUpdatePtr() {
        public void handler(osd_bitmap bitmap, int full_refresh) {
          int offs;
