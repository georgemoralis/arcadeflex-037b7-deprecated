...
public class wiping {
  ...
  public static VhConvertColorPromPtr wiping_vh_convert_color_prom =
      new VhConvertColorPromPtr() {
        public void handler(char[] palette, char[] colortable, UBytePtr color_prom) {
          ...
          for (i = 0; i < TOTAL_COLORS(0); i++) {
            colortable[Machine.drv.gfxdecodeinfo[0].color_codes_start + i ^ 3] =
                (char) ((color_prom.readinc()) & 0x0f);
          }

          /* sprites use colors 16-31 */
          for (i = 0; i < TOTAL_COLORS(1); i++) {
            colortable[Machine.drv.gfxdecodeinfo[1].color_codes_start + i ^ 3] =
                (char) (((color_prom.readinc()) & 0x0f) + 0x10);
          }
        }
      };

  public static WriteHandlerPtr wiping_flipscreen_w =
      new WriteHandlerPtr() {
   ...
   * <p>*************************************************************************
   */
  public static VhUpdatePtr wiping_vh_screenrefresh =
      new VhUpdatePtr() {
        public void handler(osd_bitmap bitmap, int full_refresh) {
          ...
          for (offs = 0x0; offs < 128; offs += 2) {
            ...
            sx =
                spriteram.read(offs + 0x100 + 1) + ((spriteram.read(offs + 0x81) & 0x01) << 8) - 40;
