...
public class kangaroo {
   ...
   *
   * <p>Kangaroo doesn't have color PROMs, the playfield data is directly converted into colors: 1
   * bit per gun, therefore only 8 possible colors, but there is also a global mask register which
   * controls intensities of the three guns, separately for foreground and background. The fourth
   * bit in the video RAM data disables this mask, making the color display at full intensity
   * regardless of the mask value. Actually the mask doesn't directly control intensity. The guns
   * are rapidly turned on and off at a subpixel rate, relying on the monitor to blend the colors
   * into a more or less uniform half intensity color.
   *
   * <p>We use three groups of 8 pens: the first is fixed and contains the 8 possible colors; the
   * other two are dynamically modified when the mask register is written to, one is for the
   * background, the other for sprites.
   *
   * <p>*************************************************************************
   */
  public static VhConvertColorPromPtr kangaroo_vh_convert_color_prom =
   ...
   * <p>*************************************************************************
   */
  public static VhStartPtr kangaroo_vh_start =
      new VhStartPtr() {
        public int handler() {
          if ((tmpbitmap = bitmap_alloc(Machine.drv.screen_width, Machine.drv.screen_height))
              == null) {
            return 1;
          }

          if ((tmpbitmap2 = bitmap_alloc(Machine.drv.screen_width, Machine.drv.screen_height))
              == null) {
            bitmap_free(tmpbitmap);
            return 1;
          }

          if ((videoram = new UBytePtr(Machine.drv.screen_width * Machine.drv.screen_height))
              == null) {
            bitmap_free(tmpbitmap);
   ...
   * *************************************************************************
   *
   * <p>Stop the video hardware emulation.
   *
   * <p>*************************************************************************
   */
  public static VhStopPtr kangaroo_vh_stop =
      new VhStopPtr() {
        public void handler() {
          bitmap_free(tmpbitmap2);
          bitmap_free(tmpbitmap);
  ...
  public static WriteHandlerPtr kangaroo_video_control_w =
      new WriteHandlerPtr() {
  ...
  public static WriteHandlerPtr kangaroo_bank_select_w =
      new WriteHandlerPtr() {
        public void handler(int offset, int data) {
  ...
  public static WriteHandlerPtr kangaroo_color_mask_w =
      new WriteHandlerPtr() {
        public void handler(int offset, int data) {
          int i;
  ...
  public static WriteHandlerPtr kangaroo_blitter_w =
      new WriteHandlerPtr() {
        public void handler(int offset, int data) {
  ...
  public static void kangaroo_plot_pixel(
      osd_bitmap bitmap, int x, int y, int col, int color_base, int flip) {
    if (flip != 0) {
      x = bitmap.width - 1 - x;
      y = bitmap.height - 1 - y;
    }
    ...
    plot_pixel.handler(
        bitmap, x, y, Machine.pens[((col & 0x08) != 0 ? 0 : color_base) + (col & 0x07)]);
  }
  ...
  public static WriteHandlerPtr kangaroo_videoram_w =
      new WriteHandlerPtr() {
        public void handler(int offset, int data) {
          ...
          if (a_G_B != 0) {
            videoram.write(
                offs, (videoram.read(offs) & 0xfc) | ((data & 0x10) >> 3) | ((data & 0x01) >> 0));
            videoram.write(
                offs + 1,
                (videoram.read(offs + 1) & 0xfc) | ((data & 0x20) >> 4) | ((data & 0x02) >> 1));
            videoram.write(
                offs + 2,
                (videoram.read(offs + 2) & 0xfc) | ((data & 0x40) >> 5) | ((data & 0x04) >> 2));
            videoram.write(
                offs + 3,
                (videoram.read(offs + 3) & 0xfc) | ((data & 0x80) >> 6) | ((data & 0x08) >> 3));
          }

          if (a_Z_R != 0) {
            videoram.write(
                offs, (videoram.read(offs) & 0xf3) | ((data & 0x10) >> 1) | ((data & 0x01) << 2));
            videoram.write(
                offs + 1,
                (videoram.read(offs + 1) & 0xf3) | ((data & 0x20) >> 2) | ((data & 0x02) << 1));
            videoram.write(
                offs + 2,
                (videoram.read(offs + 2) & 0xf3) | ((data & 0x40) >> 3) | ((data & 0x04) >> 0));
            videoram.write(
                offs + 3,
                (videoram.read(offs + 3) & 0xf3) | ((data & 0x80) >> 4) | ((data & 0x08) >> 1));
          }

          if (b_G_B != 0) {
            videoram.write(
                offs, (videoram.read(offs) & 0xcf) | ((data & 0x10) << 1) | ((data & 0x01) << 4));
            videoram.write(
                offs + 1,
                (videoram.read(offs + 1) & 0xcf) | ((data & 0x20) >> 0) | ((data & 0x02) << 3));
            videoram.write(
                offs + 2,
                (videoram.read(offs + 2) & 0xcf) | ((data & 0x40) >> 1) | ((data & 0x04) << 2));
            videoram.write(
                offs + 3,
                (videoram.read(offs + 3) & 0xcf) | ((data & 0x80) >> 2) | ((data & 0x08) << 1));
          }

          if (b_Z_R != 0) {
            videoram.write(
                offs, (videoram.read(offs) & 0x3f) | ((data & 0x10) << 3) | ((data & 0x01) << 6));
            videoram.write(
                offs + 1,
                (videoram.read(offs + 1) & 0x3f) | ((data & 0x20) << 2) | ((data & 0x02) << 5));
            videoram.write(
                offs + 2,
                (videoram.read(offs + 2) & 0x3f) | ((data & 0x40) << 1) | ((data & 0x04) << 4));
            videoram.write(
                offs + 3,
                (videoram.read(offs + 3) & 0x3f) | ((data & 0x80) << 0) | ((data & 0x08) << 3));
          }
   ...
   * <p>*************************************************************************
   */
  public static VhUpdatePtr kangaroo_vh_screenrefresh =
      new VhUpdatePtr() {
        public void handler(osd_bitmap bitmap, int full_refresh) {
          ...
          if ((kangaroo_bank_select.read() & 0x01) != 0) {
            ...
            copyscrollbitmap(
                ...
                0);
            copybitmap(
                bitmap, tmpbitmap2, 0, 0, 0, 0, Machine.visible_area, TRANSPARENCY_COLOR, 16);
          }
        }
      };
}
