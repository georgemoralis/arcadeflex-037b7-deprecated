...
public class _1943 {
  ...
  /**
   * ************************************************************************* Convert the color
   * PROMs into a more useable format.
   *
   * <p>1943 has three 256x4 palette PROMs (one per gun) and a lot ;-) of 256x4 lookup table PROMs.
  ...
  public static VhConvertColorPromPtr c1943_vh_convert_color_prom =
        ...
        public void handler(char[] palette, char[] colortable, UBytePtr color_prom) {
          ...
          for (int i = 0; i < TOTAL_COLORS(1); i++) {
            ...
            if (i % Machine.gfx[1].color_granularity == 0) {
              ...
              colortable[Machine.drv.gfxdecodeinfo[1].color_codes_start + i] =
                  (char) (color_prom.read(0) + 16 * (color_prom.read(256) & 0x03));
            }
          ...
          for (int i = 0; i < TOTAL_COLORS(3); i++) {
            ...
            colortable[Machine.drv.gfxdecodeinfo[3].color_codes_start + i] =
                (char) (color_prom.read(0) + 16 * (color_prom.read(256) & 0x07) + 128);
            color_prom.inc();
          }
  ...
  public static VhStartPtr c1943_vh_start =
      new VhStartPtr() {
        public int handler() {
  ...
  public static VhStopPtr c1943_vh_stop =
      new VhStopPtr() {
        public void handler() {
          bitmap_free(sc2bitmap);
          bitmap_free(sc1bitmap);
  ...
  public static WriteHandlerPtr c1943_c804_w =
      new WriteHandlerPtr() {
  ...
  public static WriteHandlerPtr c1943_d806_w =
      new WriteHandlerPtr() {
        public void handler(int offset, int data) {
          /* bit 4 enables bg 1 */
          sc1on = data & 0x10;
   ...
   * in the given osd_bitmap. Do NOT call osd_update_display() from this function, it will be called
   * by the main emulation engine.
   * *************************************************************************
   */
  public static VhUpdatePtr c1943_vh_screenrefresh =
      new VhUpdatePtr() {
        public void handler(osd_bitmap bitmap, int full_refresh) {
          ...
          if (sc2on != 0) {
            ...
            for (sy = 0; sy < 9; sy++) {
              int ty = (sy + top) % 9;
              UBytePtr map =
                  new UBytePtr(sc2map[ty][0], 0); // unsigned char *map = &sc2map[ty][0][0];
              offs &= 0x7fff;
          ...
          if (objon != 0) {
            ...
            for (offs = spriteram_size[0] - 32; offs >= 0; offs -= 32) {
              ...
              if (color == 0x0a
                  || color
                      == 0x0b) /* the priority is actually selected by */ /* bit 3 of BMPROM.07 */ {
          ...
          if (sc1on != 0) {
            ...
            for (sy = 0; sy < 9; sy++) {
              ...
              for (sx = 0; sx < 9; sx++) {
                ...
                int tx = (sx + left) % 9;
                UBytePtr map =
                    new UBytePtr(sc1map[ty][tx], 0); // unsigned char *map = &sc1map[ty][tx][0];
                offset = offs + (sx * 2);
