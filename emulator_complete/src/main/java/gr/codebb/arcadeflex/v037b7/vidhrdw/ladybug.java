...
public class ladybug {
  ...
  public static VhConvertColorPromPtr ladybug_vh_convert_color_prom =
  ...
  public static WriteHandlerPtr ladybug_flipscreen_w =
      new WriteHandlerPtr() {
   ...
   * <p>*************************************************************************
   */
  public static VhUpdatePtr ladybug_vh_screenrefresh =
      new VhUpdatePtr() {
        public void handler(osd_bitmap bitmap, int full_refresh) {
          ...
          /* copy the temporary bitmap to the screen */
          {
            ...
            copyscrollbitmap(
                bitmap, tmpbitmap, 32, scroll, 0, null, Machine.visible_area, TRANSPARENCY_NONE, 0);
          }
              ...
              if ((spriteram.read(offs + i) & 0x80) != 0) {
                if ((spriteram.read(offs + i) & 0x40) != 0) /* 16x16 */ {
                  drawgfx(
                      bitmap,
                      Machine.gfx[1],
                      (spriteram.read(offs + i + 1) >> 2)
                          + 4 * (spriteram.read(offs + i + 2) & 0x10),
                      spriteram.read(offs + i + 2) & 0x0f,
                      spriteram.read(offs + i) & 0x20,
                      spriteram.read(offs + i) & 0x10,
