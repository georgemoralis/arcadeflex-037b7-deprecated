...
public class wiping {
  ...
  public static StreamInitPtr wiping_update_mono =
      new StreamInitPtr() {
                  ...
                  if (channel_list[voice].oneshotplaying != 0) {
                    ...
                    if (channel_list[voice].oneshotplaying != 0) {
                      ...
                      if ((offs & 1) != 0) {
                        ...
                        mix.write(
                            0, (short) (mix.read(0) + (((w.read(offs >> 1) >> 4) & 0x0f) - 8) * v));
                        mix.offset += 2;
                      }
                    }
                  }
  ...
  public static ShStartPtr wiping_sh_start =
      new ShStartPtr() {
        public int handler(MachineSound msound) {
          ...
          for (int i = 0;
              i < last_channel;
              i++) // for (voice = channel_list; voice < last_channel; voice++)
          {
  ...
  public static ShStopPtr wiping_sh_stop =
      new ShStopPtr() {
        public void handler() {
          // mixer_table=null;
          mixer_buffer = null;
  ...
  /** ***************************************************************************** */
  public static WriteHandlerPtr wiping_sound_w =
      new WriteHandlerPtr() {
        public void handler(int offset, int data) {
          ...
          if (offset <= 0x3f) {
            for (_base = 0, voice = 0; voice < last_channel; voice++, _base += 8) {
              ...
              channel_list[voice].frequency =
                  channel_list[voice].frequency * 16
                      + ((wiping_soundregs.read(0x01 + _base)) & 0x0f);
              channel_list[voice].frequency =
                  channel_list[voice].frequency * 16
                      + ((wiping_soundregs.read(0x00 + _base)) & 0x0f);

              channel_list[voice].volume[0] = wiping_soundregs.read(0x07 + _base) & 0x0f;
              if ((wiping_soundregs.read(0x5 + _base) & 0x0f) != 0) {
                channel_list[voice].wave =
                    new UBytePtr(
                        sound_rom,
                        128
                            * (16 * (wiping_soundregs.read(0x5 + _base) & 0x0f)
                                + (wiping_soundregs.read(0x2005 + _base) & 0x0f)));
                channel_list[voice].oneshot = 1;
              } else {
                channel_list[voice].wave =
                    new UBytePtr(sound_rom, 16 * (wiping_soundregs.read(0x3 + _base) & 0x0f));
