...
public class champbas {
   ...
   *
   * <p>Pac Man has a 16 bytes palette PROM and a 128 bytes color lookup table PROM.
   *
   * <p>Pengo has a 32 bytes palette PROM and a 256 bytes color lookup table PROM (actually that's
   * 512 bytes, but the high address bit is grounded).
   *
  ...
  public static VhConvertColorPromPtr champbas_vh_convert_color_prom =
  ...
  public static WriteHandlerPtr champbas_gfxbank_w =
      new WriteHandlerPtr() {
   ...
   * <p>*************************************************************************
   */
  public static VhUpdatePtr champbas_vh_screenrefresh =
      new VhUpdatePtr() {
        public void handler(osd_bitmap bitmap, int full_refresh) {
          ...
          for (offs = spriteram_size[0] - 2; offs >= 0; offs -= 2) {
            drawgfx(
                ...
                spriteram.read(offs) & 1,
                spriteram.read(offs) & 2,
                ((256 + 16 - spriteram_2.read(offs + 1)) & 0xff) - 16,
                spriteram_2.read(offs) - 16,
                Machine.visible_area,
                TRANSPARENCY_COLOR,
                0);
          }
