...
public class exedexes {
   ...
   *
   * <p>Exed Exes has three 256x4 palette PROMs (one per gun), three 256x4 lookup table PROMs (one
   * for characters, one for sprites, one for background tiles) and one 256x4 sprite palette bank
   * selector PROM.
   *
   * <p>The palette PROMs are connected to the RGB output this way:
   *
  ...
  public static VhConvertColorPromPtr exedexes_vh_convert_color_prom =
        ...
        public void handler(char[] palette, char[] colortable, UBytePtr color_prom) {
          ...
          for (i = 0; i < TOTAL_COLORS(3); i++) {
            colortable[Machine.drv.gfxdecodeinfo[3].color_codes_start + i] =
                (char) (color_prom.read(0) + 128 + 16 * color_prom.read(256));
            color_prom.inc();
          }
        }
      };

  public static WriteHandlerPtr exedexes_c804_w =
      new WriteHandlerPtr() {
  ...
  public static WriteHandlerPtr exedexes_gfxctrl_w =
      new WriteHandlerPtr() {
        public void handler(int offset, int data) {
          /* bit 4 is bg enable */
          sc2on = data & 0x10;
  ...
  public static VhUpdatePtr exedexes_vh_screenrefresh =
      new VhUpdatePtr() {
  ...
  public static VhEofCallbackPtr exedexes_eof_callback =
