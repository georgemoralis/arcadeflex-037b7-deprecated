...
public class travrusa
{
	...
	***************************************************************************/
	...
	SET_TILE_INFO(0,travrusa_videoram.read(2*tile_index) + ((attr & 0xc0) << 2),attr & 0x0f);
		...
		if ((attr & 0x20) != 0) tile_info.u32_flags |= TILE_FLIPX;
		if ((attr & 0x10) != 0) tile_info.u32_flags |= TILE_FLIPY;
		if ((attr & 0x0f) == 0x0f) tile_info.u32_flags |= TILE_SPLIT(1);	/* hack */
	} };
	...
	***************************************************************************/
	...
	bg_tilemap.u32_transmask[1] = 0x3f; /* split type 1 has pens 6 and 7 opaque - hack! */
