...
public class polepos {
    ...
    /* interrupt states */
    static int /*UINT8*/ z80_irq_enabled = 0, z8002_1_nvi_enabled = 0, z8002_2_nvi_enabled = 0;

    /* ADC states */
    static int /*UINT8*/ u8_adc_input = 0;

    /* protection states */
    static short ic25_last_result;
    static int /*UINT8*/ u8_ic25_last_signed;
    static int /*UINT8*/ u8_ic25_last_unsigned;

    /* 4-bit MCU state */
    static class polepos_mcu_def {
        ...
        Object timer;
        /* Transfer timer */
        int coin1_coinpercred;
        /* Coinage info */
        int coin1_credpercoin;
        /* Coinage info */
        int coin2_coinpercred;
        /* Coinage info */
        int coin2_credpercoin;
        /* Coinage info */
        int mode;
        /* Mode ( 0 = read switches, 1 = Coinage, 2 = in-game ) */
        int credits;
        /* Credits count */
        int start;
        /* Start switch */
    ...
    public static InitMachinePtr polepos_init_machine = new InitMachinePtr() {
        public void handler() {
            ...
            ic25_last_result = 0;
            u8_ic25_last_signed = 0;
            u8_ic25_last_unsigned = 0;

            /* Initialize the MCU */
            polepos_mcu.enabled = 0;
            /* disabled */
            polepos_mcu.status = 0x10;
            /* ready to transfer */
            polepos_mcu.transfer_id = 0;
            /* clear out the transfer id */
    ...
    public static WriteHandlerPtr polepos_z80_irq_enable_w = new WriteHandlerPtr() {
    ...
    public static WriteHandlerPtr polepos_z8002_nvi_enable_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
    ...
    public static WriteHandlerPtr polepos_z8002_enable_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
            if ((data & 1) != 0) {
     ...
     * **********************************************************************************
     */
    public static WriteHandlerPtr polepos_adc_select_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
            u8_adc_input = data & 0xFF;
        }
    };

    public static ReadHandlerPtr polepos_adc_r = new ReadHandlerPtr() {
    ...
    public static ReadHandlerPtr polepos_io_r = new ReadHandlerPtr() {
        public int handler(int offset) {
            int ret = 0xff;
     ...
     * **********************************************************************************
     */
    public static ReadHandlerPtr polepos2_ic25_r = new ReadHandlerPtr() {
        public int handler(int offset) {
            int result;

            offset = offset & 0x3ff;
            if (offset < 0x200) {
                u8_ic25_last_signed = (offset / 2) & 0xff;
                result = ic25_last_result & 0xff;
            } else {
                u8_ic25_last_unsigned = (offset / 2) & 0xff;
                result = (ic25_last_result >> 8) & 0xff;
                ic25_last_result = (short) ((byte) u8_ic25_last_signed * u8_ic25_last_unsigned);
            }
     ...
     * **********************************************************************************
     */
    public static WriteHandlerPtr polepos_mcu_enable_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
    ...
    public static timer_callback polepos_mcu_callback = new timer_callback() {
        public void handler(int param) {
            cpu_cause_interrupt(0, Z80_NMI_INT);
        }
    };

    public static ReadHandlerPtr polepos_mcu_control_r = new ReadHandlerPtr() {
        public int handler(int offset) {
            if (polepos_mcu.enabled != 0) {
    ...
    public static WriteHandlerPtr polepos_mcu_control_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
            ...
            if (polepos_mcu.enabled != 0) {
                if (data != 0x10) {
                    /* start transfer */
                    polepos_mcu.transfer_id = data;
                    /* get the id */
                    polepos_mcu.status = 0xe0;
                    /* set status */
                    if (polepos_mcu.timer != null) {
                        timer_remove(polepos_mcu.timer);
                    }
    ...
    public static ReadHandlerPtr polepos_mcu_data_r = new ReadHandlerPtr() {
        public int handler(int offset) {
            if (polepos_mcu.enabled != 0) {
                        ...
                        switch (offset) {
                            case 0x00:
                                if (polepos_mcu.mode == 0) {
                                    return ~(readinputport(0) ^ polepos_mcu.start);
                                    /* Service, Gear, etc */
                                } else {
                                    int in;
 ...
 /*
    ...
    public static WriteHandlerPtr polepos_mcu_data_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
            if (polepos_mcu.enabled != 0) {
                //LOG(("MCU write: PC = %04x, transfer mode = %02x, offset = %02x, data = %02x\n", cpu_get_pc(), polepos_mcu.transfer_id & 0xff, offset, data ));

                if (polepos_mcu.transfer_id == 0xa1) {
                    /* setup coins/credits, etc ( 8 bytes ) */
                    switch (offset) {
                        case 1:
                ...
                if (polepos_mcu.transfer_id == 0xc1) {
                    /* set switch mode */
                    polepos_mcu.mode = 0;
                }

                if (polepos_mcu.transfer_id == 0x84) {
                    /* play sample */
                    if (offset == 0) {
                        switch (data) {
                            case 0x01:
                ...
                if (polepos_mcu.transfer_id == 0x88) {
                    /* play screech/explosion */
                    if (offset == 0) {
 ...
 /* 0x7n = Screech sound. n = pitch (if 0 then no sound) */
                ...
                if (polepos_mcu.transfer_id == 0x81) {
                    /* Set coinage mode */
                    polepos_mcu.mode = 1;
                }
            }
    ...
    public static WriteHandlerPtr polepos_start_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
            data &= 1;
